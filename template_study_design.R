...
get.effect.power <- 
  function(totaln = 200, # total sample size to enroll for all arms and stages together
        ...
        message_logistic = tryCatch(error = function(e)e, warning = function(w) w,
                                    {
                                      ...
                                      model = glm(y~arm+astage, family=quasibinomial, data=data.model)
...
# get.effect.power(totaln=720,event.rates=c(0.2, 0.1),stages=4,ntimes=50,pick=1,drift=1.2)
...
# test_off_22 = get.effect.power(totaln=2000,event.rates=c(0.2, 0.15, 0.1, 0.05),stages=4,ntimes=500,pick=1,drift=0.8)
# 
...
data = read.csv(paste0(filepath, 'extreme_2_arm.csv'))
...
save.image(paste0(filepath, 'GLM_sandwich_extreme_2_arm_results.RData'))
